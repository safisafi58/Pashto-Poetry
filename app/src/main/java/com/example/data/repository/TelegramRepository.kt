...
class TelegramRepository(context: Context) {
    ...
    suspend fun fetchChannelPosts(token: String? = null, channelId: String? = null): List<TelegramPost> = withContext(Dispatchers.IO) {
        ...
        // 1. Try Telegram Bot API getUpdates if token is provided
        ...
        // 2. Try Web Channel Preview HTML parsing if posts list is empty
        ...
        // 3. Fallback Pashto poetry posts from Telegram channel
        if (posts.isEmpty()) {
